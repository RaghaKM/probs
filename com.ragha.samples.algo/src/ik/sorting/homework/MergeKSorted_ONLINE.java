...
/*Got this solution from following URL : 
 * https://ideone.com/mEsVvI
 * The point i had ignored/overlooked was that, we need to pick the next element from the same array which the heap belongs to. 
 */
public class MergeKSorted_ONLINE {
		
		static void mergeKSortedArrays(int[][] arr, int k) {
			Node[] hArr = new Node[k];
			int resultSize = 0;
			for(int i=0;i<arr.length;i++) {
				Node node = new MergeKSorted_ONLINE().new Node(arr[i][0],i,1);
				hArr[i] = node;
				resultSize += arr[i].length;
			}
		
			MinHeap mh = new MergeKSorted_ONLINE().new MinHeap(hArr);
